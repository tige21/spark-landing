...
- **Performance:** the 3D hero (`three` / r3f, ~890 KB) is a lazy chunk loaded **only** on desktop + WebGL + motion-allowed. Mobile, reduced-motion, and no-WebGL get the static engraving poster — three.js is never downloaded. LCP = the hero poster image.
